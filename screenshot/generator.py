...
class ScreenshotGenerator:
    ...
    def _save_frame_to_jpeg(self, frame, infohash_hex: str, timestamp_str: str):
        ...
        :param infohash_hex: torrent 的 infohash，用于文件名。
        :param timestamp_str: 帧的时间戳字符串，用于文件名。
        """
        # 创建一个唯一的文件名，并将时间戳中的冒号替换为连字符，以确保文件名有效
    ...
    def _decode_and_save(self, extradata: Optional[bytes], packet_data: bytes, infohash_hex: str, timestamp_str: str):
        ...
        try:
            ...
            # 如果解码器需要更多数据才能输出一个完整的帧，第一次调用 decode 可能会返回一个空列表。
            ...
            if not frames:
                ...
                try:
                    ...
                    # 在刷新时，如果解码器内部没有更多数据，可能会抛出 EOFError，这是正常现象。
                    pass
